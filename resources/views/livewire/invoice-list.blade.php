    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-x-4">
            ...
            @if ($yearFilter || $search || $paymentStatusFilter)
                <button wire:click="resetFilters" class="items-center gap-x-2 flex bg-[#e8e8e8] pr-4 pl-3 py-2 text-sm rounded-[4px] shadow transition">
            ...
            <a href="{{ route('fatture.nuova') }}" wire:navigate class="items-center gap-x-2 flex bg-black text-white px-4 py-2 text-sm rounded-[4px] shadow transition">
        ...
        <table class="w-full text-sm text-left border-collapse">
            <thead>
                <tr class="text-[#616161] text-xs border-b">
                    <th class="py-2 pl-2 pr-4 font-normal">Numerazione</th>
            ...
            <tbody class="divide-y">
                @foreach ($invoices as $invoice)
                    <tr class="hover:bg-gray-50 group">
                        <td class="whitespace-nowrap py-4 pl-2 pr-4 font-normal text-gray-800">
                            <div class="flex items-center gap-2">
                                <img src="{{ $invoice->numbering->logo  }}" alt="{{ $invoice->invoice_number }} logo" class="w-8 h-8 rounded-full">
                        ...
                        <td class="whitespace-nowrap py-4 px-4 text-gray-900 font-medium">{{ number_format($invoice->netto_post_tax, 2, ',', '.') }} EUR</td>
                        <td class="whitespace-nowrap py-4 px-4 text-gray-700">{{ $invoice->issue_date->format('d/m/Y') }}</td>
                        <td class="whitespace-nowrap py-4 px-4 text-gray-700"> {{ $invoice->deadline_date ? $invoice->deadline_date->format('d/m/Y') : '–' }}</td>
